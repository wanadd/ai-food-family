#!/usr/bin/env python3
"""Build holiday/kids recipe steps update JSON from enrichment JSONL files.

Reads successful enrichment artifacts, validates the merged result, and writes
an update JSON plus a Markdown report. It does not read or write the database.

Usage:
    python backend/scripts/build_holiday_kids_steps_update.py
"""
...
DEFAULT_INPUTS = [
    ROOT / "exports" / "holiday_kids_steps_enriched_12.jsonl",
    ROOT / "exports" / "holiday_kids_steps_enriched_retry_6.jsonl",
]
DEFAULT_OUTPUT_PATH = ROOT / "exports" / "holiday_kids_steps_update_12.json"
DEFAULT_REPORT_PATH = ROOT / "reports" / "holiday_kids_steps_update_report.md"
EXPECTED_COUNT = 12
SOURCE = "steps_enrichment_holiday_kids_v1"
FORBIDDEN_PHRASES = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build holiday/kids steps update JSON")
    parser.add_argument(
        "--input",
        action="append",
        default=None,
...
def normalized_steps(value: Any) -> list[str]:
    if not isinstance(value, list):
        return []
    return [str(step or "").strip() for step in value if str(step or "").strip()]
...
def validate_record(record: dict[str, Any]) -> list[str]:
    ...
    steps = normalized_steps(raw_steps)
    if len(steps) != len(raw_steps):
        errors.append("blank_step")
    if len(steps) < 5:
        errors.append("steps_count_less_than_5")

    all_steps = normalize_text(" ".join(steps))
...
def build_update_record(record: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "steps": normalized_steps(record["steps"]),
        "source": SOURCE,
    }
...
def build_report(
    ...
    valid_id_values: set[int] = set()
    for record in records:
        try:
            valid_id_values.add(int(record.get("id")))
        except (TypeError, ValueError):
            continue

    lines = [
        "# Holiday Kids Steps Update Report",
        "",
        "Scope: build update JSON from holiday/kids steps enrichment outputs. No database changes and no apply/import were performed.",
        "",
        "## Inputs",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            f"- Unique records: `{len(valid_id_values)}`",
...
def main() -> int:
    ...
    invalid_record_ids = {id(item["record"]) for item in invalid_records}
    valid_records = [
        record for record in records if id(record) not in invalid_record_ids
    ]
    ...
    can_write = (
        ...
        and len(id_counts) == EXPECTED_COUNT
