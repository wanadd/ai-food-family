...
export function ScreenBackNav({
  ...
  const linkClass =
    "inline-flex items-center gap-1 text-sm font-semibold text-sage-700 transition hover:text-sage-800";
    ...
    <>
      ...
      {allowFloating && scrolled ? (
        <button
          type="button"
          onClick={goBack}
          className="fixed right-4 z-50 flex min-h-[44px] items-center gap-1.5 rounded-pill border border-cream-border bg-cream-surface/95 px-4 py-2.5 text-sm font-semibold text-sage-700 shadow-lift backdrop-blur-md active:scale-[0.98]"
