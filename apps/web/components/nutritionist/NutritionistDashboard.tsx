...
import {
  ...
  type CareToggleId,
  type CareToggles,
} from "@/lib/nutritionist/care-storage";
...
const CARE_ITEMS: { id: CareToggleId; label: string }[] = [
  { id: "water", label: "Вода" },
...
export function NutritionistDashboard() {
  ...
  function toggleCare(id: CareToggleId) {
    setCare((prev) => {
      const next = { ...prev, [id]: !prev[id] };
      saveCareToggles(next);
      return next;
    });
  }
      ...
      <main className="mx-auto max-w-lg space-y-3 px-4 py-4">
        ...
        <section className="rounded-2xl border border-stone-100 bg-white p-4 shadow-sm">
          ...
          <p className="mt-0.5 text-xs text-stone-500">
            Напоминания скоро — пока можно отметить интерес
          </p>
