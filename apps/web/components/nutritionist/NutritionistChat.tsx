...
export function NutritionistChat({
  ...
  return (
    ...
    >
      <h2 className="text-sm font-bold text-graphite-900">Спросить нутрициолога</h2>
      <p className="mt-1 text-xs text-graphite-500">
        {formatAmaCost(amaAskCost)} за ответ · у вас {formatAmaCost(amaBalance)}
      </p>

      {chatError ? (
        <div className="mt-2 rounded-control border border-warm/30 bg-warm/10 px-3 py-2 text-sm text-graphite-900">
          <p>{chatError}</p>
          {/Лимит|Амов?|Ам |тариф|Пробный/i.test(chatError) ? (
            <Link
              href="/subscription"
              className="mt-1.5 inline-block font-semibold text-sage-800"
            >
      ...
      <div
        ref={listRef}
        className="mt-3 max-h-52 space-y-2 overflow-y-auto rounded-control bg-cream-deep/50 p-3"
      >
        {messages.length === 0 ? (
          ...
          messages.map((msg) => (
            <div
              ...
              className={`rounded-control px-3 py-2 text-sm ${
                msg.role === "user"
                  ? "ml-6 bg-sage-600 text-white"
                  : "mr-4 bg-cream-surface text-graphite-800 shadow-soft"
              }`}
            >
              {msg.text}
            </div>
      ...
      >
        <input
          ...
          disabled={sending}
          className="min-h-[44px] flex-1 rounded-control border border-cream-border bg-cream-surface px-3 text-sm text-graphite-900 outline-none focus:border-sage-400 focus:ring-2 focus:ring-sage-200"
        />
        <button
          type="submit"
          disabled={sending || !input.trim()}
          className="pa-btn-primary min-h-[44px] shrink-0 px-4 text-sm disabled:opacity-50"
        >
          {sending ? "…" : "→"}
        </button>
      </form>
