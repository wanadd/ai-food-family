...
export function FamilyManageSheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0">
      <div className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white p-4 pb-8">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-bold text-stone-900">Управление семьёй</h2>
          ...
          <>
                  ...
                  {telegramMembers.map((m) => (
                    <li key={m.id}>
                      <button
                        ...
                        className="w-full rounded-xl border border-stone-200 px-3 py-2.5 text-left text-sm disabled:opacity-50"
                      >
                        {m.display_name}
                        {transferId === m.id ? " …" : ""}
                      </button>
            ...
            {!confirmDelete ? (
              <button
                ...
                className="mt-6 w-full rounded-xl border border-red-200 py-3 text-sm font-semibold text-red-700"
              >
                Удалить семью
              </button>
            ) : (
