...
Компонент внутренних вкладок: `components/layout/SegmentedTabs.tsx`. В Меню он монтируется через `components/menu/MenuSubTabs.tsx` (в MenuHub и в `components/menu/MenuSectionLayout.tsx`). Покупки — Этап 3.
...
После Этапа 2 каталог рецептов живёт на `/menu/recipes` (вкладки Меню), а `/recipes` инвертирован и мягко ведёт туда. Деталь `/recipes/[id]` остаётся. `/pantry`, `/menu/leftovers` остаются живыми до Этапа 3. Циклов нет.
