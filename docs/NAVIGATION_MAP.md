...
|---|---|---|---|---|
| 1 | Меню | `/menu` | `/menu/*`, `/recipes/*` | внутренние вкладки (Этап 2) |
| 2 | Покупки | `/shopping` | `/shopping/*`, `/pantry/*` | центр закупок; внутренние вкладки (Этап 3) |
...
- **Меню:** Моё меню (`/menu`) · Рецепты (`/menu/recipes`) · Избранное (`/menu/favorites`) · Коллекции (`/menu/collections`).
...
- **Покупки:** Покупки (`/shopping`) · Запасы (`/shopping/pantry`) · Остатки (`/shopping/leftovers`).

Компонент внутренних вкладок: `components/layout/SegmentedTabs.tsx` (монтируется в Этапах 2–3).
...
| `/menu/recipes` | `/recipes` | временный (до Этапа 2) |
| `/menu/favorites` | `/recipes` | временный (до Этапа 2) |
| `/menu/collections` | `/recipes` | временный (до Этапа 2) |
| `/menu/scenarios` | `/recipes` | временный |
| `/shopping/pantry` | `/pantry` | временный (до Этапа 3) |
| `/shopping/leftovers` | `/menu/leftovers` | временный (до Этапа 3) |

`/recipes`, `/pantry`, `/menu/leftovers` остаются живыми до Этапов 2–3. Инверсия временных редиректов произойдёт после переезда контента (циклов нет: в Этапе 1 редирект идёт только новое → старое).
...
UX-архитектура Покупок уже предусматривает место под будущее действие «Заказать продукты» (см. комментарий в `SHOPPING_SUBTABS`, `nav-config.ts`). В Этапе 1 доставка не реализуется: без API, без оплаты, без интеграций магазинов.
